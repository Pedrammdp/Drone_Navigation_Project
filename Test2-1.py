...
# Load the image
img = cv.imread('Pics/test2-1.png')
cv.imshow('image', img)
...
lower_color = np.array([160, 114, 185])  # replace with the lower HSV values of the gate color
upper_color = np.array([179, 255, 255])  # replace with the upper HSV values of the gate color
...
edges = cv.Canny(dilated, 150, 300)
...
for i, contour in enumerate(contours_filtered):
    ...
    contour_img = cv.drawContours(np.zeros_like(img), [contour], 0, (0, 255, 0), 2)
